...
variable_ranges = {
    ...
    "Te_SG": (0.02, 0.10),
    # Line parameters
    "Rline_SG": (0.01, 1.0), "Lline_SG": (0.01, 1.0),
    # Load parameters
...
def get_user_inputs():
    ...
    ibr_tab, sg_tab, line_tab, load_tab = st.sidebar.tabs([
        "IBR (Droop)", "SG", "Line Parameters", "Load Parameters"
    ])
    ...
    # IBR Parameters
    with ibr_tab:
        ibr_params = [param for param in variable_ranges.keys()
                     if not param.endswith('_SG') and param not in ['Rload', 'Lload', 'Rx']
...
def prepare_simulation_parameters(user_params):
    ...
    for key, value in user_params.items():
        if key.endswith('_SG'):
            if key.startswith('Rline') or key.startswith('Lline'):
...
def visualization(testResults):
    state_variables = [
        "Po(IBR1)", "Qo(IBR1)", "phid(IBR1)", "phiq(IBR1)", "gammad(IBR1)", "gammaq(IBR1)",
        "iid(IBR1)", "iiq(IBR1)", "vcd(IBR1)", "vcq(IBR1)", "iod(IBR1)", "ioq(IBR1)",
        "theta", "wr(SG1)", "psid(SG1)", "psiq(SG1)", "Eq1(SG1)", "Ed1(SG1)", "psi1d(SG1)",
        "psi2q(SG1)", "P1(SG1)", "Pg(SG1)", "Pf(SG1)", "P2(SG1)", "vx(SG1)", "Efd(SG1)",
        "ilineD(LineSG)", "ilineQ(LineSG)", "iloadD(Load)", "iloadQ(Load)"
    ]
