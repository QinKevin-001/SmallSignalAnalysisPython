...
variable_ranges = {
    ...
    "wcPLLPlant": (float(round(2 * np.pi * 50, 2)), float(round(2 * np.pi * 1000, 2))),
    "wcPlant": (float(round(2 * np.pi * 0.1, 2)), float(round(2 * np.pi * 5, 2))),
    ...
    "KpL": (0.1, 10.0),        "KiL": (0.1, 1000.0),
    "KpS": (0.1, 10.0),        "KiS": (0.1, 1000.0),
    "KpC": (0.1, 10.0),        "KiC": (0.1, 1000.0),
    "wcPLL": (float(round(2 * np.pi * 50, 2)), float(round(2 * np.pi * 1000, 2))),
    "wc": (float(round(2 * np.pi * 1, 2)), float(round(2 * np.pi * 20, 2)))
}
...
def run_simulation(user_params):
    return case05main_gflPlant_infinite.main_gflPlant_infinite(user_params)

def visualization(testResults):
    state_variables = [
        "thetaPlant", "epsilonPLLPlant", "wPlant", "epsilonP", "epsilonQ", "PoPlant", "QoPlant", "PsetDelay",
        "QsetDelay", "theta", "epsilonPLL", "wf", "Po", "Qo", "phid", "phiq", "gammad", "gammaq", "iid", "iiq", "vcd",
        "vcq", "iod", "ioq"
    ]
