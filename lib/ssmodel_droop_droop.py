...
def ssmodel_droop_droop(wbase, parasIBR1, parasIBR2, parasLoad, dominantParticipationFactorBoundary):
    ...
    Asys = np.block([
        [A1 + Bw1 @ Cw1.T + B1 @ Ngen1 @ C1, B1 @ Ngen2 @ C2, np.zeros((13, 2))],  # Ensure last term is (13,2)
        [Bw2 @ Cw1.T + B2 @ Ngen1 @ C1, A2 + B2 @ Ngen2 @ C2, np.zeros((13, 2))],  # Ensure last term is (13,2)
        [Bloadw @ Cw1.T + Bload @ Ngen1 @ C1, Bload @ Ngen2 @ C2, Aload + Bload @ Nload]  # Should be (2,2)
    ])
