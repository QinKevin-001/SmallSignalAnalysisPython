...
def ssmodel_droopPlant_infinite(wbase, parasIBR, dominantParticipationFactorBoundary):
    ...
    x0 = np.array([0, 1])
    # Options similar to Matlab's levenberg-marquardt with no display
    opts = {'xtol': 1e-6, 'maxfev': 500, 'factor': 0.1}  # (unused but provided for clarity)
    ...
    # Assigning labels to the state variables (set second column to 'IBR' for all states)
    if isinstance(ssVariables, list):
        ssVariables = [list(row) for row in ssVariables]  # ensure mutable lists
